...
功能:
  在仿真环境中加载训练好的策略模型,测试机器人走路效果
  相当于"质检员"角色,检验训练成果是否达标

在机器人走路中的作用:
  1. 加载训练好的策略模型(policy.pt)
  2. 在仿真环境中让机器人执行策略
  3. 观察机器人走路是否稳定、摔倒多少次
  4. 生成对比数据(Sim vs Real)
  5. 可选:生成仿真视频
...
使用场景:
  ...
  2. 对比不同训练迭代的模型
  3. 生成仿真动画视频
  4. 与真实机器人数据对比分析(Sim2Real验证)

使用方式:
  python play.py --experiment q2 --num_envs 1 --time 10
  
  --experiment: 实验名称(对应experiments/目录下的子目录)
  --num_envs: 并行环境数量(测试时通常设为1)
  --time: 测试时长(秒)
  --iter: 指定加载特定迭代的模型(如--iter 1000)
  --video: 是否保存仿真画面为视频
  --debug: 是否输出详细调试信息
  --cmp_real: 是否与真实机器人数据对比
...
代码结构:
  ...
  │   1. 加载配置和模型                                                    │
  │      ├─ 读取cfg.yaml获取机器人配置                                   │
  │      ├─ 加载policy.pt权重到Actor网络                                 │
  │      └─ 关闭所有随机化(确保测试可复现)                                │
  │                              ↓                                         │
  │   2. 创建仿真环境(单环境,确定性)                                     │
  │      └─ LeggedRobotEnv + GymEnvWrapper                               │
  │                              ↓                                         │
  │   3. 主测试循环                                                       │
  │      ┌─────────────────────────────────────────────────────────────┐  │
  │      │  for epoch in range(测试次数):                            │  │
  │      │    obs = env.reset()        # 重置环境                     │  │
  │      │    for step in range(最大步数):                           │  │
  │      │      act = actor(obs)      # 推理得到动作                  │  │
  │      │      obs, rew, done = env.step(act)  # 执行动作          │  │
  │      │      if done: break       # 摔倒则结束                   │  │
  │      │    记录:走路时长、摔倒次数、奖励                          │  │
  │      └─────────────────────────────────────────────────────────────┘  │
  │                              ↓                                         │
...
与其他文件的关系:
  - train.py: 生成policy.pt,play.py加载并测试
  - export_pt2onnx.py: 导出ONNX后可在真实机器人上运行
...
def play(args):
    ...
    流程:
    1. 加载训练好的策略模型
    2. 创建仿真环境(关闭随机化,保持确定性)
    3. 让机器人执行策略,记录数据
    4. 可视化结果或与真实数据对比
    """
    device = args.rl_device  # 推理设备
    ...
    debug_dir = join(exp_dir, 'debug')  # 调试数据目录
