...
【完整训练流程】
  ┌─────────────────────────────────────────────────────────────┐
  │  1. 加载配置 (config/Base.py)                              │
  │                          ↓                                   │
  │  2. 创建仿真环境 (env/legged_robot.py)                      │
  │                          ↓                                   │
  │  3. 加载任务 (env/tasks/birl_task.py)                       │
  │                          ↓                                   │
  │  4. 初始化策略网络 (model/simple_policy.py)                 │
  │                          ↓                                   │
  │  ┌─────────────────────────────────────────────────────┐   │
  │  │           训练主循环 (迭代5000次)                   │   │
  │  │                                                    │   │
  │  │  ┌─────────────────────────────────────────────┐  │   │
  │  │  │  经验收集 (24步/迭代)                       │  │   │
  │  │  │  obs→Actor→action→step→reward→storage       │  │   │
  │  │  └─────────────────────────────────────────────┘  │   │
  │  │                    ↓                              │   │
  │  │  ┌─────────────────────────────────────────────┐  │   │
  │  │  │  PPO更新 (3 epoch/迭代)                      │  │   │
  ...
  │  └─────────────────────────────────────────────────────┘   │
  │                          ↓                                │
  │  5. 输出模型: experiments/xxx/model/policy.pt            │
  └─────────────────────────────────────────────────────────────┘
...
【完整调用关系图】
          ...
          ▼                               ▼                               ▼
┌───────────────────┐         ┌───────────────────┐         ┌───────────────────┐
│   config/Base.py  │         │ env/utils/*.py    │         │   utils/*.py      │
│   (配置参数)       │         │   (工具函数)       │         │   (通用工具)       │
└───────────────────┘         └───────────────────┘         └───────────────────┘
                                          │
          ┌───────────────────────────────┼───────────────────────────────┐
          ...
          ▼                               ▼                               ▼
┌───────────────────┐         ┌───────────────────┐         ┌───────────────────┐
│ env/legged_robot.py│◄────────│ env/GymEnvWrapper │────────►│ env/tasks/        │
│   (物理仿真)        │         │   (环境包装)       │         │   birl_task.py    │
└─────────┬─────────┘         └───────────────────┘         └─────────┬─────────┘
          │                                                           │
          ▼                                                           ▼
┌───────────────────┐                                       ┌───────────────────┐
│    IsaacGym       │                                       │PhaseModulator     │
│  (NVIDIA物理引擎)  │                                       │  (步态相位管理)    │
└───────────────────┘                                       └───────────────────┘
                                          │
          ┌───────────────────────────────┼───────────────────────────────┐
          ...
          ▼                               ▼                               ▼
┌───────────────────┐         ┌───────────────────┐         ┌───────────────────┐
│  model/*.py       │         │   rl/alg/ppo.py   │         │ tensorboard       │
│  (策略网络)        │         │   (PPO算法)        │         │   (日志记录)       │
│  - simple_policy  │         │   - GAE计算        │         └───────────────────┘
│  - actor/critic   │         │   - 策略梯度       │
└───────────────────┘         └───────────────────┘
...
├─────────────────────────────────────────────────────────────────────────┤
│  config.Base              → 动态导入配置类                               │
│  env.utils.get_args       → 解析命令行参数                               │
│  env.utils.helpers        → 更新配置、设置种子、解析仿真参数              │
│  env.LeggedRobotEnv       → 创建物理仿真环境                             │
│  env.GymEnvWrapper        → 封装环境为Gym风格接口                        │
│  env.tasks.load_task_cls  → 加载任务类(BIRLTask)                        │
│  model.load_actor         → 加载策略网络                                 │
│  model.load_critic        → 加载价值网络                                 │
│  rl.alg.PPO              → PPO算法实现                                  │
│  utils.yaml.ParamsProcess → 配置保存与加载                               │
│  torch.utils.tensorboard  → 训练日志可视化                               │
└─────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────┐
│ 第二层: 间接调用(通过GymEnvWrapper)                                     │
├─────────────────────────────────────────────────────────────────────────┤
│  BIRLTask.action()        → 处理网络输出为关节指令                       │
│  BIRLTask.step()          → 更新任务状态                                 │
│  BIRLTask.observation()   → 获取观察向量                                 │
│  BIRLTask.reward()        → 计算奖励(核心!)                              │
│  BIRLTask.terminate()     → 判断终止条件                                 │
│  LeggedRobotEnv.step_torques() → 执行物理仿真                           │
│  LeggedRobotEnv.step_states()  → 更新状态张量                            │
└─────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────┐
│ 第三层: 深层调用(通过BIRLTask)                                           │
├─────────────────────────────────────────────────────────────────────────┤
│  PhaseModulator           → 步态相位调制器                               │
│  env.utils.math           → 数学工具函数                                 │
│  torch.norm/exp/clip等    → PyTorch张量操作                              │
└─────────────────────────────────────────────────────────────────────────┘

【文件依赖层次】

Level 0: train.py (主入口)
    │
    ├── Level 1: 配置与工具
    │   ├── config/Base.py
    │   ├── env/utils/get_args.py
    │   ├── env/utils/helpers.py
    │   └── utils/yaml.py
    │
    ├── Level 1: 环境层
    │   ├── env/legged_robot.py
    │   │   └── IsaacGym API
    │   ├── env/GymEnvWrapper.py
    │   │   └── env/tasks/birl_task.py
    │   │       └── env/utils/phase_modulator.py
    │   └── env/tasks/__init__.py
    │
    ├── Level 1: 模型层
    │   ├── model/__init__.py
    │   ├── model/simple_policy.py
    │   └── model/tcn.py (可选)
    │
    └── Level 1: 算法层
        └── rl/alg/ppo.py
...
【输出】
  ...
  - experiments/{name}/model/cfg.yaml: 训练配置备份
"""
import importlib
import os
